...
export async function isUsernameTaken(username){
    ...
    try{
        ...
        return await db.collection("users").findOne({username:username},{projection:{username:1,_id:0}})
    }
    catch(err){
...
export async function LoginToUser(email, role) {
    ...
    try {
        client = await MongoClient.connect(process.env.MONGO_URI);
        let db =  client.db(process.env.DB_NAME);
       return await db.collection("users").findOne({ email: email , role:role },{projection:{email:1,pass:1,password:1,_id:1}})
    }
    catch (err) {   
